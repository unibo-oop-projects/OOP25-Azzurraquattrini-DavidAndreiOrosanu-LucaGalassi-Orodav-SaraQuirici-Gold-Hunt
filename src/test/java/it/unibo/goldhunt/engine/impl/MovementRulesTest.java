...
public class MovementRulesTest {
    ...
    void mustStopOnShouldReturnFalseForInvalid() {
        ...
        assertFalse(rules.mustStopOn(pos(9, 9), player));
    ...
    void pathCalculationShouldReturnEmptyWhenInvalid() {
        ...
        assertTrue(rules.pathCalculation(pos(9, 9), pos(0, 0), player).isEmpty());
        assertTrue(rules.pathCalculation(pos(0, 0), pos(9, 9), player).isEmpty());
    ...
    void isReachableShouldReturnFalseForPositionsInvalid() {
        final Player player = new BasicPlayer(pos(0, 0));
        final MovementRules rules = new MovementRulesImpl(new TestBoard(5));
        assertFalse(rules.isReachable(pos(7, 7), pos(0, 0), player));
        assertFalse(rules.isReachable(pos(0, 0), pos(7, 7), player));
    }
